...
class SearchEngine:
    ...
    def _load_models(self):
        ...
        import json
        with open(MODELS_DIR / "lgbm_features_speed.json", "r") as f:
            self.lgbm_features = json.load(f)
            
        self.lgbm = lgb.Booster(model_file=str(MODELS_DIR / "lgbm_model_speed.txt"))
    ...
    def search(
        ...
        # 2. Candidate Retrieval (BM25 Recall)
        ...
        # Blend exact lexical and fuzzy subword recall.
        ngram_weight = 0.38 if mode == "balanced" else 0.50
        scores = scores_word + ngram_weight * scores_ngram
        
        top_indices = np.argsort(scores)[-candidates_to_rerank:][::-1]
        
        candidates = self.corpus.iloc[top_indices].copy().reset_index(drop=True)
        candidates["search_term_norm"] = q_norm
        candidates["search_term_tok"] = [q_tok] * len(candidates)
        
        logger.info(f"Retrieved {len(candidates)} candidates. Extracting features on the fly...")
